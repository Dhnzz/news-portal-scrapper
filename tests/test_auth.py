...
async def test_login_success_sets_session_cookie_and_accesses_home(client, make_user):
...
async def test_login_failure_wrong_password(client, make_user):
    await make_user("admin", "admin123")

    response = await _login(client, "admin", "salah")
    assert response.status_code == 401
    assert "news_session" not in client.cookies


async def test_login_failure_unknown_user(client):
    response = await _login(client, "tidak-ada", "admin123")
    assert response.status_code == 401
    assert "news_session" not in client.cookies


async def test_logout_clears_session(client, make_user):
    await make_user("admin", "admin123")
...
async def test_login_page_redirects_when_already_logged_in(client, make_user):
    await make_user("admin", "admin123")
    await _login(client, "admin", "admin123")
